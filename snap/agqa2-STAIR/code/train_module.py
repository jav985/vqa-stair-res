...
try:
    ...
    print("--- Set PyTorch multiprocessing sharing strategy to file_system ---")
except RuntimeError:
    print("--- INFO: PyTorch multiprocessing sharing strategy might already be file_system or setting failed ---")
    pass

def set_worker_sharing_strategy(worker_id: int) -> None:
    torch.multiprocessing.set_sharing_strategy("file-system")
...
def main(args):
    ...
    if args.dataset == 'AGQA':
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn, pin_memory=False)
        valid_dataloader = DataLoader(valid_dataset, batch_size=1, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn, pin_memory=False)
    else:
